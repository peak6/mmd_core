...
decode_obj(<<?FAST_STRING, Rest/binary>>) ->
    {Sz, Rest2} = tag_size(Rest),
...
decode_obj(<<?FALSE,Data/binary>>) ->
    {false, Data};
decode_obj(<<?FAST_BYTES, Rest/binary>>) ->
    {Sz, Rest2} = tag_size(Rest),
    <<Bytes:Sz/binary,Rem/binary>> = Rest2,
    {?bytes(Bytes),Rem};
...
decode_obj(<<?BYTE, B, Rest/binary>>) ->
    {?byte(B),Rest};
decode_obj(<<?FAST_ARRAY, Rest/binary>>) ->
    {Sz, Rest2} = tag_size(Rest),
    decode_array(Rest2,Sz,[]);
decode_obj(<<?FAST_MAP, Rest/binary>>) ->
    {Sz, Rest2} = tag_size(Rest),
    decode_map(Rest2,Sz,[]);
...
downgrade(v1_0, <<?BYTE, B, Rest/binary>>) ->
    {<<?BYTE, B>>, Rest};
downgrade(v1_0, <<?FAST_STRING, Rest/binary>>) ->
    {Sz, Rest2} = tag_size(Rest),
    <<Str:Sz/binary, Rest3/binary>> = Rest2,
    {[?VARINT_STRING, mmd_encode:encode_varint(Sz), Str], Rest3};
...
downgrade(v1_0, <<?FAST_MAP, Rest/binary>>) ->
    {Sz, Rest2} = tag_size(Rest),
    {MapData, Rest3} = downgrade_map(v1_0, Rest2, Sz, []),
    {[?VARINT_MAP, mmd_encode:encode_varint(Sz), MapData], Rest3};
downgrade(v1_0, <<?FAST_ARRAY, Rest/binary>>) ->
    {Sz, Rest2} = tag_size(Rest),
    {ArrayData, Rest3} = downgrade_array(v1_0, Rest2, Sz, []),
    {[?VARINT_ARRAY, mmd_encode:encode_varint(Sz), ArrayData], Rest3};
downgrade(v1_0, <<?FAST_BYTES, Rest/binary>>) ->
    {Sz, Rest2} = tag_size(Rest),
...
upgrade(v1_0, <<?VARINT_STRING, Data/binary>>) ->
    {Sz, PostSz} = decode_varint(Data),
    <<Str:Sz/binary, Rest/binary>> = PostSz,
    {mmd_encode:mk_tag_with_size(?FAST_STRING, Sz, Str), Rest};
...
upgrade(v1_0, <<?VARINT_MAP, Bin/binary>>) ->
    ...
    {mmd_encode:mk_tag_with_size(?FAST_MAP, Sz, MapData), Rest};
upgrade(v1_0, <<?VARINT_ARRAY, Bin/binary>>) ->
    {Sz, Rest} = decode_varint(Bin),
    {ArrayData, Rest2} = upgrade_array(v1_0, Rest, Sz, []),
    {mmd_encode:mk_tag_with_size(?FAST_ARRAY, Sz, ArrayData), Rest2};
upgrade(v1_0, <<?VARINT_BYTES, Data/binary>>) ->
    {Sz, PostSz} = decode_varint(Data),
    <<Bytes:Sz/binary, Rest/binary>> = PostSz,
    {mmd_encode:mk_tag_with_size(?FAST_BYTES, Sz, Bytes), Rest};
upgrade(v1_0, <<?VARINT_ERROR, Bin/binary>>) ->
    {ErrCode, PostErr} = decode_svarint(Bin),
    {Body, Rest} = upgrade(v1_0, PostErr),
    {mmd_encode:mk_tag_with_size(?FAST_ERROR, ErrCode, Body), Rest}.
...
tag_size(Rest) ->
    case Rest of
        <<1:4, SSz:4, Sz:SSz/unsigned-unit:8, Rest2/binary>> ->
            {Sz, Rest2};
        <<0:4, SSz:4, Sz:SSz/signed-unit:8, Rest2/binary>> ->
            {Sz, Rest2}
    end.
